...
| **OD7** | Как enforce'ить границу Orch↔Integrator (Оркестратор по умолчанию её поглощает) | **async-протокол** `request→await-fix→resume`: capability-spec в `pending-actions.md`, **не чинить сам**. Реализация — Process Fabric (DEC-DEV-0171): §6 BLOCK паркует линию (`awaiting_capability` P7 / `awaiting_capability_impl` P5), ingest несёт capability-spec payload'ом в PA-запись гейта; resume = PA→done → `pa-scan --tick` → обычный bracket-ре-ран (P5 продолжает с недоделанных tasks.md-чекбоксов). ScheduleWakeup-ожидание отброшено — durable-парковка Fabric переживает сессии | **построено (DEC-DEV-0171) + live-прогон 2026-07-10 = условный GO судьи** (парковка/payload/resume/continuation PASS — R2-R5,R7,R8; отчёт `dev/process-fabric/OD7_LIVE_RUN_GRADE_REPORT.md`); дефекты прогона пофикшены DEC-DEV-0174 (ingest bracket-guard DEF-OD7-2, charter-fallback blocked+go_gate:null DEF-OD7-3, probe-presence из `.env` DEF-OD7-1, терминал `closed_without_runtime` + owner-гард ANOM-OD7-1/2); R1 re-run целевым путём `/orchestrator:run --fabric` — pending; история: гипотеза RUN 01, анти-эталон #29, эталон #10 |
