...
> **Статус:** модуль **построен** — P1–P8 + Epic E (`deploy-to-stage` / `rollback-release`); 9 процессов на диске (`orchestrator/processes/*.mjs`), последний инкремент P8 `user-journey-acceptance` — DEC-DEV-0225 (2026-07-23). Пер-процессная готовность (включая substrate-gated ноги P5/P7) — каталог процессов §3.2; статусы решений OD1–OD10 — таблица §9 (в т.ч. §6-канал = OD7 построен и live-валидирован, OD10 решён DEC-DEV-0075). Этот документ — исходный **дизайн-харвест** `v1.0-draft (dogfood RUN 01)` (2026-06-14, DEC-DEV-0073; сессия `6dc62bc8`, см. `dev/ORCHESTRATOR_DOGFOOD_RUN_01.md`, прожившая цепочку D2-T → D3-impl → validation → runtime); разделы **[RUN 01]** эмпирически обоснованы прогоном. Живой статус — [ROADMAP «Где мы сейчас»](../../ROADMAP.md#где-мы-сейчас).
...
| **P8** | `user-journey-acceptance` | D3+ acceptance | приёмка user-journey после `DEPLOYED` (перед `done`) | — | ✅ построена (DEC-DEV-0225): детерминированные Playwright-журнеи `tests/uja/*.spec.ts` против staging-URL → PASS/FAIL/ENV_NOT_READY; + нога **визуальной конформности** (DEC-DEV-0237): Screen Inventory активных MK каждой `has_ui`-фичи × скриншоты `<artifacts>/visual/<MK-id>/SI-n.*` × явные `tests/uja/visual-skips.json` → COMPLETE / COMPLETE_WITH_SKIPS / INCOMPLETE / N/A, на DoD-прогоне INCOMPLETE = жёсткий блок; живой прогон VM-gated |
...
| **OD7** | Как enforce'ить границу Orch↔Integrator (Оркестратор по умолчанию её поглощает) | **async-протокол** `request→await-fix→resume`: capability-spec в `pending-actions.md`, **не чинить сам**. Реализация — Process Fabric (DEC-DEV-0171): §6 BLOCK паркует линию (`awaiting_capability` P7 / `awaiting_capability_impl` P5), ingest несёт capability-spec payload'ом в PA-запись гейта; resume = PA→done → `pa-scan --tick` → обычный bracket-ре-ран (P5 продолжает с недоделанных tasks.md-чекбоксов). ScheduleWakeup-ожидание отброшено — durable-парковка Fabric переживает сессии | **построено (DEC-DEV-0171) + LIVE-ВАЛИДИРОВАН по совокупности критериев** — прогон 2026-07-10 (условный GO: R2-R5,R7,R8 PASS; `dev/process-fabric/OD7_LIVE_RUN_GRADE_REPORT.md`) + фиксы дефектов DEC-DEV-0174 (ingest bracket-guard, charter-fallback, probe `.env`, owner-гард) + **R1 re-run 2026-07-11 = PASS** (park machine-backed полным ledger-брекетом диспетчера; судья-вердикт в `OD7_R1_RERUN_BRIEF.md` §Outcome, DEC-DEV-0175; попутно закрыт DEF-OD7-CLOSE — owner-выход `evt:owner.close` со всех парковок, charter v4). Остаток: R6/`evt:env.up` — event-gated; история: гипотеза RUN 01, анти-эталон #29, эталон #10 |
