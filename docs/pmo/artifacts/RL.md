...
| 3 | Stage | `DEPLOYED × READY` (healthcheck 2xx) на текущем составе релиза; rollback этого контура проверен (`ROLLED_BACK` хотя бы раз); **UJA PASS** на текущем составе релиза (P8 `user-journey-acceptance`: user-journey реально проходят против staging, а не только `/health` отвечает); **для первого релиза — UJA PASS на realistic-input** (DoD-прогон P8 с `dodRun: true`, `input_profile: realistic` в `run.json`; директива владельца 2026-07-23 / DEC-DEV-0231: «видео 30 мин – 2 ч», 5-сек фикстуры — только dev; DoD-прогон на dev-профиле = честный `ENV_NOT_READY`, не PASS) | deploy/rollback `run.json` + P8 `run.json` (`uja_result: PASS` × `input_profile: realistic`) | ❌ |
...
| 5 | Prod-готовность | Гейт production-готовности субстрата пройден; **оговорки-отступления (`concerns[]` из P5/P6 — с DEC-DEV-0231 обязательный deviations-отчёт implementer'а: усечённые пороги/расписания, значения «со слов клиента», mock-стенды) отсутствуют или явно приняты владельцем**; Rollout/Rollback plan заполнены содержательно (не «просто задеплоить»); **для `has_ui`-релизов** (DEC-DEV-0237): P8 DoD-прогон `visual_evidence ∈ {COMPLETE, COMPLETE_WITH_SKIPS}` (skip'ы из `visual-skips.json` предъявляются владельцу строкой evidence на approve-гейте impl-sync (Step 4a) и принимаются вердиктом `C`/`D` — носитель ратификации = `impl_sync.visual_review`) **И** каждая `has_ui` FM несёт `impl_sync.visual_review` (V-23; авто-diff MK — v1.1) | гейт-док субстрата + это тело RL + P8 `run.json` (`visual_evidence`) + FM frontmatter `impl_sync.visual_review` | ❌ |
