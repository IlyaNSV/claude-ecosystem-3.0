...
- **`has_ui: false` ≠ авто-decline (DEC-DEV-0231; M10 §3.2).** Потребность в NFR определяет РИСК
  операции, не «продуктовость» фичи: внутренняя/back-office фича с мутирующими операциями над
  чужими доменами (блокировка аккаунта, takedown контента) несёт риск не ниже пользовательской.
  Decline такой фичи требует явного risk-обоснования в `nfr_decline_reason` — «внутренняя,
  мало пользователей» обоснованием не является (прецедент M10 §3.2: «отклик ≤ нескольких
  секунд», «best-effort без SLA» — неверифицируемые формулировки вместо NFR).
