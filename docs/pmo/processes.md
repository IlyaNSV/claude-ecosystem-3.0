...
6. **F.5a NFR Review** (условный, opt-in)
   ...
   **F.5a.0 Ask**
   ...
   - **Риск, не «продуктовость», определяет потребность в NFR (DEC-DEV-0231; M10 §3.2):**
     `has_ui: false` / «внутренняя» / «back-office» — НЕ основание рекомендовать [D] declined.
     Фича с мутирующими операциями над чужими доменами (блокировка аккаунта, takedown чужого
     контента, изменение чужих данных) несёт риск не ниже пользовательской — при таких
     операциях ассистент рекомендует [Y], а decline требует явного risk-обоснования в
     `nfr_decline_reason` (прецедент M10 §3.2: у двух непользовательских фич `nfr: []`
     обернулся неверифицируемыми формулировками «≤ нескольких секунд», «best-effort»).
