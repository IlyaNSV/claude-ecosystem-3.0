...
**Ни один артефакт не создаётся человеком с чистого листа.** Все 21 тип артефактов создаётся через унифицированный паттерн:
...
- **C2 `confidence:` field в frontmatter** — все 22 типа артефактов имеют `confidence: high | medium | low` + `confidence_notes`. ИИ обязан articulate уверенность при approve. Tied with A1 auto-approve и meta-feedback usage stats.
