...
| **Hooks** | Drift detection, журналирование | `.claude/hooks/integrator-*.js` |
...
- Этапы: profile → propose → approve → install → configure → contract → verify
- Каждый этап approve-gate
...
- Детальнее: §12 Environment Scanner
...
### Фаза 1 — Read-only (безопасно)
- `/integrator:research`
- `/integrator:map`
- `/integrator:gaps`
- `/integrator:status`
- `/integrator:journal`
- Skills: research protocol, profiling
- Subagent: tool-researcher

**Что получаем:** можно изучать инструменты, видеть gaps, ничего не ломая. Уже полезно.

### Фаза 2 — Installation
- `/integrator:add`
- `/integrator:remove`
- `/integrator:replace`
- Skills: installation protocol, contract design
- Hooks: integrator-journal-hook (логирует каждое добавление)

**Что получаем:** можно подключать инструменты под PMO. Пилотировать на cc-sdd.

### Фаза 3 — Maintenance
- `/integrator:update`
- `/integrator:verify`
- `/integrator:debug`
- Hooks: drift-detector (SessionStart)

**Что получаем:** долгоживущая инфраструктура. Обновления, починки.

### Фаза 4 — Export для Orchestrator
- `/integrator:docs`
- Спецификация tool-docs.md формата
- Генерация шаблонов

**Что получаем:** готовый фундамент для будущего Orchestrator Module.
...
│   └── drift-detection.md
└── hooks/
    ├── integrator-journal.js          # PostToolUse для записи решений
    ├── integrator-drift-check.js      # SessionStart для drift detection
    └── integrator-contract-validate.js # PreToolUse для блокировок
```
...
**OQ-I12:** Отношения с существующими внешними конфигами → **ЗАКРЫТО (2026-04-17):** встроенный механизм анализа (см. §12 Environment Scanner).

**OQ-I13:** Язык tool-docs → **ЗАКРЫТО (2026-04-17):** универсальный технический английский, по стилю — API reference документация (см. §13 Tool-Docs Style Guide).
...
- [ ] Пилотный инструмент выбран (предложение: cc-sdd)
...
### 12.3. Output: Environment Baseline
...
### 12.4. Алгоритм детекции конфликтов
...
### 12.5. «Guess purpose» для пользовательских файлов
...
## 13. Tool-Docs Style Guide (OQ-I13)
...
### 13.5. Пример: минимальный tool-docs для cc-sdd
...
## 14. MCP-стэк и расширенные примитивы Claude Code
...
### 14.1. Core MCP-стэк (обязателен для полноценной работы)
...
### 14.2. Рекомендуемый расширенный стэк
...
### 14.3. Ситуативный стэк (по необходимости)
...
### 14.4. Отклонённые (не нужны Integrator'у)
...
### 14.5. Расширенные примитивы Claude Code
...
### 14.7. Стратегия MCP-availability
