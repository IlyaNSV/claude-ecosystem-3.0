...
### 8.4. Internal capability provider — санкционированный fallback (DEC-DEV-0218)

Тезис экосистемы «tool-agnostic делегирование D3-D6 во внешние инструменты через handoff» —
**дефолт, а не абсолют**. Когда рыночный скан не даёт внешнего инструмента, проходящего
критерии зоны (прецедент D3-05/D3-06: деплой на self-hosted VM — рынок agentic-деплоя
либо PaaS-locked, либо preview/experimental, либо без agentic-гейтинга; DEC-INT-RESEARCH-0002 →
DEC-INT-0016), Integrator вправе оснастить зону **internal-инструментом** (role-агент +
процесс Orchestrator'а), при обязательных условиях:

1. **Тот же контракт, что у внешнего:** CNT-файл (producer/consumer), профиль в
   `active-tools.yaml` с явным `source/type: internal`, те же verify/audit-циклы.
   Internal ≠ бесконтрактный.
2. **Research-first сохраняется:** решению предшествует рыночный скан с зафиксированным
   в журнале инстанса shortlist'ом и причинами отклонения внешних кандидатов.
3. **Событийный re-audit рынка:** в маппинге зоны фиксируется триггер пересмотра
   (пример D3-05/06: «пилот контейнеризуется / появляется Docker-стек ⇒ переоценить
   Kamal-класс инструментов как механический слой под нашими гейтами»). Internal-провайдер —
   это выбор «лучшее сейчас», а не навсегда.
4. **Граница §8.3 не размывается:** internal-провайдер оснащает (authoring), исполнение —
   за Orchestrator; те же строки таблицы выше.
