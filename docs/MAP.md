...
>
> **Интерактивные карты (HTML, генерируются из SSOT):** [командно-артефактная карта](guide/ecosystem-map.html) · [**BPMN-карта всех процессов**](guide/ecosystem-processes.html) — drill-down lane→процесс→шаг, zoom, свёртка узлов вместе со связями, слои рёбер. Mermaid-диаграммы ниже — для быстрого coarse-обзора; интерактивный граф — для «досконально всех процессов».
>
> **Руководство оператора (USER-слой):** [docs/guide/README.md](guide/README.md) — единый вход «Начни здесь»: лестница понимания L0→L5, роутер «Я хочу…», обе интерактивные карты. Эта карта (docs/MAP) — coarse entry-point; guide — как **делать работу**.
