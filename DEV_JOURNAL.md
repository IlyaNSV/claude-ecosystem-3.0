...
### Outcome
(заполняется по ходу) Карта + индекс созданы и закоммичены на ветку `docs/user-guide`; `01-first-session` следом в том же PR. Параллельная сессия во время review-workflow переключила ветку mid-run — верифицировано, что pre-switch tip ≡ текущий main (пустой diff), tool-ошибок чтения ноль, все drift-находки держатся на текущем main.
