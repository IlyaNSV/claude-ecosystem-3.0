...
## DEC-DEV-0190 — CRLF-хрупкость G36 wiring-теста (deep-discovery-agents): frontmatter-парсер падал на Windows-checkout

**Тип:** fix (root cause + lesson).

**Симптом.** После merge G36 (`agents/product/{market-researcher,competitor-analyst}.md`, DEC-DEV-0186) в другую feature-ветку `npm run verify` давал 6 ✗ в `tests/product/deep-discovery-agents-wiring.test.cjs` («frontmatter 'name' missing/empty», «Cannot read properties of undefined (reading 'split')»), тогда как **прямой** `node …deep-discovery….cjs` проходил. Набор падений плавал (то market-researcher, то competitor-analyst).

**Root cause.** `.gitattributes` пинует к LF только `*.mjs/*.cjs/*.js/*.sh` — **не `*.md`**. Новые `agents/*.md` при checkout на Windows (`core.autocrlf`) получают **CRLF** на диске (git хранит LF → `git diff` чист, `git status` пуст → невидимо). Тест-helper `frontmatter()` был написан под LF: (1) `/^---\n…\n---/` не матчит `---\r\n` → весь блок `null`; (2) даже с этим — `m[1].split('\n')` оставляет `\r` в хвосте строк, а `^([a-z_]+):\s*(.*)$` не проходит через `\r` (в JS `.` и `$` без флагов не включают `\r`) → поля пусты. `model` парсился «случайно» — последняя строка frontmatter теряла `\r` при захвате внешним `\r?\n---`. Verify в самой G36-ветке был зелёным потому, что там файлы записаны агентом с LF; хрупкость латентна до первого CRLF-checkout (=любая Windows-машина пользователя).

**Fix.** Сделал парсер EOL-агностичным (как и должен быть тест, проверяющий контент): `/^---\r?\n([\s\S]*?)\r?\n---/` + `split(/\r?\n/)`. Поведение на LF 1:1, на CRLF — теперь корректно. `npm run verify` EXIT=0.

**Сознательно НЕ делал.** Не добавлял `*.md text eol=lf` в `.gitattributes`: корневой фикс класса, но требует `git add --renormalize` → массовый diff по сотням `.md`, конфликтующий с активной параллельной deadweight-сессией. Defensive тест-fix узок и достаточен; `.gitattributes`-правило для `.md` — кандидат отдельного изолированного PR, когда параллельный трек закрыт.

**Lessons.**
1. **Тесты, читающие текстовые артефакты, обязаны быть EOL-агностичны** — `\r?\n`, не `\n`; иначе зелёный CI на LF-машине автора маскирует красный на Windows-checkout пользователя. Класс-родич FB-LR-27 (CRLF ломал `.mjs`-launcher), но там источник пинован `.gitattributes`, а `.md` — нет.
2. **«Прямой запуск проходит, в цепочке — нет» — сигнал не про порядок тестов, а про состояние файла на диске** (здесь EOL применился checkout'ом между прогонами). Диагностика через `cat -A` (увидеть `^M$`), не через `git diff` (нормализация прячет).
