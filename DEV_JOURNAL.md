...
## DEC-DEV-0068 — Orchestrator dogfood RUN 01: реверс-инжиниринг 13ч-сессии → каталог из 7 процессов + SPEC v1.0-draft
...
## DEC-DEV-0069 — Адаптер handoff-to-ccsdd: фикс §10-клоббера upstream + блокирующий C-07 (канонизация P0-1)

**Date:** 2026-06-14
**Trigger:** Follow-up №1 из DEC-DEV-0068 (Orchestrator dogfood RUN 01). Harvest показал: пилот поймал и починил баг адаптера, но эталон экосистемы (`adapters/handoff-to-ccsdd.js`) **всё ещё его нёс** — фикс не доехал upstream (класс дрейфа DEC-DEV-0065).
**Tag:** #bug-fix #pilot-finding #tooling

### Context

Root cause полностью разобран в DEC-DEV-0068 (P0-1): `extractSections` ключевал секции плоско по `## N.` (last-write-wins); v1.3/v1.4 handoff'ы встраивают MK/DS/NM UI-подсекции под §10 с рестартом нумерации `## 1.`–`## 7.`, тихо затирая реальные §1/§2/§5/§6. Контракт при этом рапортовал `passed: true` — presence-level `C-04` проверял лишь наличие `## N.` заголовка, не семантику маппинга. Это **единственный дефект прогона, тихо прошедший синтаксический гейт**.
...
- Остаются follow-up'ы DEC-DEV-0068: P0-2 `gate-risk-classifier` (#2), scope-решение OD10 (#3), нативный P5 (#4), прогон №2 на D3 (#5).

---

## DEC-DEV-0070 — OD10 решён: первый инкремент Оркестратора = E2+E4 (P3 + нативный P5), override harvest-рекомендации

**Date:** 2026-06-14
**Trigger:** DEC-DEV-0068 follow-up #3 (S4). Требовалось зафиксировать scope первого build-инкремента модуля перед прототипированием — он определяет, что строить (P3 vs P3+P5).
**Tag:** #scope-change #architecture #orchestrator

### Context

RUN 01 (DEC-DEV-0068) раскрыл цепочку из 7 процессов (P1–P7). Concept §8 знал только `route-handoff-to-cc-sdd` (= P3, зона E2/D2-T). Прогон ушёл до E4 (P5 TDD-impl) — D3-кода. Вопрос: первый инкремент = только E2 (минимум, не нагружает непроверяемый §6) или расширить до E4 (полный pipeline до кода)?
