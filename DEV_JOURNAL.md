...
### Outcome
Реализовано на ветке `chore/session-audit-pending-2026-06-16` (checkpoint-коммит). Зелёно: `tests/product/cascade-scalar.test.cjs` — 14 ассертов (scalar-формат / list-направление / conflict / no-op / draft), подключён в `npm run verify`; +1 функциональный кейс в `smoke-hooks.js` (26/26). При финализации — снять в журнале пометку DEC-DEV-0023 «v1.2 deferral для MK↔SC».
