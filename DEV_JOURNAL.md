...
### Outcome
(заполняется по ходу) Карта + индекс + **`01-first-session` walkthrough** (нарратив bootstrap→init→plan→feature→handoff с реальными approve-гейтами, извлечён из закопанного `processes.md §8/§9`) созданы и закоммичены на ветку `docs/user-guide` (2 коммита, один PR). Параллельная сессия во время review-workflow переключила ветку mid-run — верифицировано, что pre-switch tip ≡ текущий main (пустой diff), tool-ошибок чтения ноль, все drift-находки держатся на текущем main. Осталось по плану: `00-concepts`, end-to-end, Tier 0 (дрейф README/SPEC), Tier 2 (генератор).
