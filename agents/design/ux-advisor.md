...
## What you may read
Read (read-only): all `.product/` artifacts incl. `.product/mockups/` (MK), `.product/navigation/` (NM), the design system (DS), the app map (AM); prior `.product/.advisor-findings/` + `.product/.da-findings/`; `.claude/docs/`. You SHOULD NOT modify files, produce mockups, or speculate about author motivation.
