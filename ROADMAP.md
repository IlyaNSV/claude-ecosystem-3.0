...
> ✅ **BUILT 2026-07-11 (DEC-DEV-0176).** Фактический скоуп ≠ буквальный список ниже: скиллы drift-detection/tool-docs-generator уже были построены Phase 5 (фаза их потребляет, не создаёт), debug-protocol инлайнен в `debug.md`; «full drift-detection» урезан до local-only модели DEC-DEV-0045 (разделяемая либа `hooks/integrator/lib/drift-checks.cjs` + SessionStart-хук `drift-check.js`); `verify --light`/periodic — cut v1.1+. SSOT решений/cuts — DEV_JOURNAL 0176. Runtime smoke S1-S5 — `dev/gates/PHASE_7_SMOKE_TEST_PLAN.md` (next VM-визит). Секция ниже — исторический план.
