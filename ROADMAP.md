...
> **Последнее обновление:** 2026-07-17 (поздн.) — **Релиз 1.12.2 cut: трек когерентности репо ЗАКРЫТ по волнам A+B+C (аудит DEC-DEV-0220, 48 подтверждённых находок).** Волна A (PR #224): флип strict extended-видов `check-counts` (0220-a) + 6 копий ship-default гейта уроков приведены к коду (0220-b) + фикс поставки «version Unreleased» в install.sh/.ps1/bootstrap + вердикты гейтов схлопнуты в указатели (0220-c). Решения D-1..D-4 приняты по мандату + правки трека на диск `dev/coherence/VERIFIED_FIXES.md` (0220-d, PR #225). Волны B+C (0220-e, PR #226): все готовые кластеры реестра применены (terminology D3-D5 / spec-vs-impl связкой replace⏳+provision / stale-refs / PHASE_4 R1 / B.1 ×4 скилла + тело PS к канону с Why-now); **V-09 перенесён на исполнимый чекпойнт D1.4a** (был невыполним по построению И обратную ссылку SEG←VP не писал никто — vp-design получил обязательный backfill-шаг); **S2 link-checker strict в цепи `verify`** (`check:links`, 93% измеренной точности, адверсариально проверен, 10 битых ссылок починено); **S5 кросс-файловая precedence с границей** (12 дублей обязательств схлопнуты; словарь CHANGELOG = `Added|Changed|Fixed`, D-3). Остаток трека — событийный: S4-чекер карты принуждения, S6-чекер B.1 (`creates:`-frontmatter + фантомы Related Skills), 3 незарегистрированных B.1-нарушителя (MR/CA/AM). Хвост DEV_JOURNAL = **0220-e**.
>
