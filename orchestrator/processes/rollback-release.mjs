...
const swap = await agent(
  ...
  `1) Resolve the CURRENT release and the PRIOR releases/<ts> (the newest release older than current) from the on-disk layout. 2) If there is NO prior release to revert to, STOP — return swapped:false, no_prior:true (the bad release stays live, flagged; the owner decides — do NOT delete it, do NOT fabricate a release). 3) Otherwise atomically flip \`current\` to the prior release and \`systemctl restart\` the @app/* units.\n` +
