...
> **Дата:** 2026-07-17 · **Аудит гонялся на:** `docs/seam-handoff-final` (`cd298a3`) · **Исполнение:** ветка `fix/coherence-sweep` от `origin/main` (`2f82bce`).
> **Что это:** массовый поиск мест, где репозиторий противоречит сам себе, с независимой adversarial-верификацией каждой находки.

---

## 0. Статус исполнения — **ТРЕК ЗАКРЫТ 2026-07-17** (остаток событийный)

> **Реестр приведён в соответствие с фактом 2026-07-28** (гигиена репо). Прежняя редакция этого
> раздела объявляла себя «живым» и держала S2/S3/S5/S7 как «⬜ не начат», а 43 находки — как
> «не начаты», хотя всё перечисленное было исполнено волнами A/B/C ещё 2026-07-17. Раздел
> тронут последний раз коммитом `daaf873` — **до** волн B+C (`a172a4b`), поэтому и разошёлся.
>
> **⚠ Реестр снят на другой ветке, чем шло исполнение** (аудит — `docs/seam-handoff-final`,
> правки — от `origin/main`), поэтому координаты строк в §6+ дрейфуют, а часть находок была
> закрыта параллельной работой. Для оставшихся (S4/S6) правило прежнее: **перед правкой
> проверяй, жива ли находка на `origin/main`** (прецедент: «43 команды» в гиде уже не
> воспроизводилась).

**SSOT исполнения — журнал: DEC-DEV-0220-a..e + шов трека `dev/coherence/SEAM.md` (`status: CLOSED`).**
Поставка — релиз `1.12.2`. Ниже — сверка механизмов с коммитами.

### Механизмы

| # | Механизм | Статус | Доказательство / примечание |
|---|---|---|---|
| **S1** | `check-counts` → 6 kinds | ✅ **СДЕЛАН** (DEC-DEV-0220) | флип extended warn→strict исполнен: `de60546` (0220-a), тумблер `COUNTS_EXTENDED_STRICT=0` |
| **S7** | хелпер версии CHANGELOG + тест | ✅ **СДЕЛАН** | `c739b93` (0220-c) — `install.sh` больше не печатает «version Unreleased»; CHANGELOG `[1.12.2]` |
| **S3** | pointer-collapse вердиктов гейтов | ✅ **СДЕЛАН** | тот же `c739b93` — inline-копии вердиктов схлопнуты в указатели на §Outcome брифа |
| **S2** | link-checker | ✅ **СДЕЛАН, STRICT** | `a172a4b` (0220-e) — `dev/meta-improvement/scripts/check-links.cjs` в цепи `npm run verify` (`check:links`); тумблер `LINK_CHECK_STRICT=0` |
| **S5** | снять самоограничение D12 | ✅ **СДЕЛАН** | `a172a4b` (0220-e) — precedence в `CLAUDE.md` стала кросс-файловой (с явной границей предметных канонов), 12 дублей схлопнуты |
| **S4** | генератор карты принуждения | ⬜ **не начат** — событийный остаток | Кластер warn→strict он уже не «убивает»: 6 копий приведены к коду вручную (`c54ceb8`, 0220-b, решение владельца). Ценность механизма теперь — анти-рецидив, а не разбор долга |
| **S6** | чекер конвенции B.1 | ⬜ **не начат** — событийный остаток | Нужен явный маркер «скилл создаёт артефакт X» (`creates:`-frontmatter). Незарегистрированные нарушители сверх реестра: `market-research-protocol-quick.md`, `competitive-analysis-protocol-quick.md`, `app-map-generate.md` |

### Правки находок

| Группа | Статус |
|---|---|
| Счётчики (`counts-1/2/4/5` + 1 новая, найденная S1) | ✅ **5 исправлено** — README 9→13, product SPEC 22→23 и 12→13 hooks, integrator SPEC 12→13, design SPEC 5→7 |
| Кластер `lesson-presence-gate` warn→strict (6 копий прозы) | ✅ **исправлен** — `c54ceb8` (0220-b) |
| Кластеры terminology (4) · spec-vs-impl (5) · stale-refs (5 live + precedence-dup-5) · B.1 ×4 скилла · `PHASE_4` R1 → `[FIXED]` | ✅ **применены** — `a172a4b` (0220-e), 5 opus-субагентов по реестру `dev/coherence/VERIFIED_FIXES.md` |
| V-09 (SEG↔VP) — перенос чекпойнта на исполнимый момент | ✅ **исполнен** — `a172a4b` (0220-e), D-4 |
| Битые ссылки, переоткрытые чекером S2 (10 шт.) | ✅ **починены** в ходе постройки `check-links` (0220-e) |
| Остаток находок | **не пересчитывался поштучно.** Незакрытое сгруппировано в S4/S6 выше — это и есть событийный хвост трека |

**Что осталось (событийно, не блокеры):** S4 и S6. Вход — warn-only (инвариант 4 трека), отдельными
единицами работы, по триггеру, а не по расписанию.

**Снятая директива.** Прежний абзац «Что НЕ делать: не править кластер warn→strict вручную до S4»
**отменён исполнением**: владелец принял решение привести 6 копий к коду руками (`c54ceb8`,
DEC-DEV-0220-b) до постройки S4. Директива сохранена здесь только как история — исполнять её нельзя,
она блокирует уже сделанное.
