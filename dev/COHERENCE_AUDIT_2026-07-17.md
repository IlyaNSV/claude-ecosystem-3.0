...
## 0. Статус исполнения (живой раздел — обновлять по мере правок)

**⚠ Реестр снят на другой ветке, чем идёт исполнение.** Аудит гонялся на `docs/seam-handoff-final`, правки идут от `origin/main`. Часть находок могла быть закрыта параллельной работой — **перед правкой каждой проверяй, жива ли она на `origin/main`** (прецедент: находка «43 команды» в гиде на `origin/main` уже не воспроизводится). Координаты строк тоже могли сдвинуться.

### Механизмы

| # | Механизм | Статус | Примечание |
|---|---|---|---|
| **S1** | `check-counts` → 6 kinds | ✅ **СДЕЛАН** (DEC-DEV-0220) | warn-only; `--strict-extended` зелёный ⇒ **флип возможен, решение владельца** |
| **S4** | генератор карты принуждения | ⬜ не начат | Убивает весь кластер warn→strict (5 находок). Приоритет №1 из оставшихся |
| **S7** | хелпер версии CHANGELOG + тест | ⬜ не начат | Чинит единственную настоящую ошибку поставки (`install.sh` печатает «Unreleased») |
| **S6** | чекер конвенции B.1 | ⬜ не начат | Нужен явный маркер «скилл создаёт артефакт X» |
| **S2** | link-checker | ⬜ не начат | Детект-онли; нужен whitelist внешних URL/плейсхолдеров |
| **S3** | pointer-collapse вердиктов гейтов | ⬜ не начат | **Механизм НЕ строить** — разовая правка структуры |
| **S5** | снять самоограничение D12 | ⬜ не начат | **Механизм НЕ строить** — декларация + разовый sweep |
...
| Остальные 43 находки | ⬜ не начаты |

**Порядок для продолжения** (обоснование — в §5): сначала **S4** и **S7** (механизмы схлопывают находки, после них часть правок отпадает сама), затем ручные кластеры **S3** (вердикты гейтов) и `CLAUDE.md`-кластер (6 находок, auto-loaded — самый дорогой по последствиям), потом остальное.

**Что НЕ делать:** не править кластер warn→strict вручную до S4 — руками правится 6 копий, которые снова разойдутся при следующем флипе. Механизм сначала, правки следом.
