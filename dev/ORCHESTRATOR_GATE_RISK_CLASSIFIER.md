...
> **Статус:** `WIRED` (S5b, DEC-DEV-0072). Материализован: предикат `orchestrator/lib/gate-risk-classifier.cjs` + регрессия-тест против таблицы §6 (**17/17** с M5) + регламент `skills/orchestrator/gate-risk-classifier.md`; подключён в P5 `feature-to-tdd-impl` (роутинг HIGH→independent kiro-review / LOW→inline-verify). Этот файл — design-источник (предикат §3/§4, реестр §5, валидация §6). Открытые вопросы §10 (per-project деривация реестра, M5 A/B, declarative-vs-imperative на schema+логике) — для live-прогона на фиче иного класса.
