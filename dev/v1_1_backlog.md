...
## Протокол контекстных швов (Context Seam Protocol) — процессный механизм, КАНДИДАТ

**Originally planned:** Not planned — surfaced 2026-07-12 user idea
**Deferred:** 2026-07-12 per DEC-DEV-0196 (зафиксирован как кандидат; решение о принятии НЕ принято)
**Awakened:** 2026-07-14 per **DEC-DEV-0202** — сработал bring-forward №1 (реальный эпизод потери контекста: сессия S2 `c4525579`). Вариант A исполнен: шаблон шва **v2** + SEAM-ACK-правило в проектном `CLAUDE.md` + warn-хуки `seam-reinject-compact`/`seam-freshness-warn`; инстансы **2/3** (`dev/context-audit/SEAM.md`, `dev/semantic-continuity/SEAM.md`). Глобальный уровень (N2→MDP) и промоушен в D7-паттерн — после валидации R10, решение владельца.
**Defer rationale:** Механизм адресует реальный класс проблем (деградация длинного контекста + накопление stale-убеждений), но: (а) его доказательная база — непроверенная посылка о кривой деградации модели; (б) §5 деферред-дока честно допускает, что механизм **избыточен** — autoflow уже требует резать работу на коммитируемые единицы, и всё «новое» может схлопнуться до 4 строк в существующих контрактах; (в) 0 задокументированных инстансов (для промоушена в D7-паттерн нужно 3+). Кодификация до evidence = ритуал вместо механизма. В `CLAUDE.md` и память ассистента намеренно НЕ внесён.
**Bring-forward trigger:** любое из:
- Реальный эпизод потери контекста (переспросил решённое / «вспомнил» файл в устаревшей форме / повторил отброшенную ветку).
- Многодневная задача, заведомо не влезающая в одно окно (**Epic E — естественный кандидат**: цепочка E.A→E.B→E.C→E.D на живой VM).
- Авто-компактация сработала посреди работы, после неё обнаружился дрейф состояния.
- Владелец решает провести замер (OQ-CS-01) — нужен протокол для плеча «со швом».

### Architectural intent

Полный контекст (правила R1-R11, шаблон шва, анти-хайп раздел, 7 открытых вопросов, критерии принятия/отклонения, 3 варианта кодификации) — **`dev/deferred/CONTEXT_SEAM_PROTOCOL.md`**. Здесь — только реестровая выжимка.

Суть: заменить недостающее свойство модели (ровная эффективность по всему окну) инженерным приёмом **checkpoint/restart** — резать длинную работу на safepoint'ах (границах логических единиц из autoflow), передавая в новую сессию **верифицируемый** шов вместо надежды на lossy авто-компактацию. Дуален к `Recovery after interruption` (тот — про НЕзапланированный разрыв, этот — про запланированный).

Реально ново поверх существующих механизмов — **только 4 вещи** (§5 деферред-дока):
- **N1** temp-шов для НЕзавершённой работы (autoflow покрывает только завершённую единицу — коммит);
- **N2** context firewall: расход окна как третье основание для делегирования (дополняет MDP, где основания — цена и качество);
- **N3** «не начинай то, что заведомо не влезет в остаток окна»;
- **N4** re-anchoring критических инвариантов перед важным решением.

### Implementation notes

**Блокирующая деталь дизайна (OQ-CS-03):** scratchpad **session-specific** — новая сессия не найдёт шов, положенный туда. Нужно стабильное предсказуемое место (`dev/.seam/CURRENT.md` в gitignore / файл в корне / ручная передача пути). Без этого механизм ломается на первом применении.

**Неразведанное (OQ-CS-02):** отдаёт ли Claude Code текущее заполнение контекста хукам/statusline. Не проверено. Если нет — правило чисто поведенческое (класс autoflow), порог не enforced.

**Путь при пробуждении (рекомендация):** вариант A (4 строки N1-N4 в существующие контракты, ~1 ч) на одном реальном длинном эпизоде → сбор 2-3 инстансов → только потом решать про полный протокол + D7-чеклист. Это же и есть путь к промоушену в паттерн по `patterns/README.md`.

### References to existing spec

- `dev/deferred/CONTEXT_SEAM_PROTOCOL.md` — полный контекст (SSOT инициативы)
- `CLAUDE.md` (глобальный) §«Recovery after interruption» — дуальный механизм; §«Model Delegation Policy» — точка вплетения N2
- `CLAUDE.md` (проектный) §«Autoflow» (DEC-DEV-0100) — единица синхронизации = safepoint
- `dev/meta-improvement/patterns/README.md` — процедура промоушена (3+ инстанса)
- DEC-DEV-0196 — решение о фиксации без активации

### Estimated effort при возврате

- Разведка OQ-CS-02 + решение OQ-CS-03 + шаблон: ~1 ч
- Вариант A (минимальный): ~1 ч · Вариант B (полный протокол + D7-чеклист): +1-2 ч · Вариант C (с автоматизацией): зависит от OQ-CS-02
- Сбор инстансов: событийно
