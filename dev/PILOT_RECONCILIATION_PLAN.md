...
| 2 | **App Map (AM)** | `019bf5e`, `c561dc1` | `docs/pmo/artifacts/AM.md` (24-й тип, singleton), `commands/design/map.md`, `skills/design/app-map-generate.md`, 6× `hooks/design/app-map-*.js` (в manifest только `app-map-cascade`), V-AM-* в `design-artifact-validate.js`, правки design-manifest и `artifacts/README.md` | ⬜ требует DEC: канонизация типа, счётчики 23→24 (sweep ~10 доков, список — reference_pmo_canonical_counts) |
| 3 | **open-design generator (CNT-004)** | `edf7057` (DEC-INT-0012) | `skills/design/open-design-workflow.md`, dispatch-строка в `design-session.md`, instance-адаптеры `od-mcp-call.cjs` / `od-consolidate.cjs` / `od-fidelity-check.js` (канонических источников в repo `adapters/` НЕТ — нарушение tri-location DEC-DEV-0040) | ⬜ требует DEC: канон сейчас viewer-only (CNT-003, 0063); решить generator vs viewer-only; `design_tool` enum в MK.md |
...
| `docs/pmo/artifacts/README.md` | LESSON → «23 типа» | AM → «23 типа» | ⚠️ **разные 23!** Честный merge = 24 (Шаг 2). Счётчики сверять списком, не числом |
...
- [x] **Шаг 1 — worktree pre-flight upstream** (этот PR, DEC-DEV-0065).
- [ ] **Шаг 2 — App Map extraction.** Отдельный DEC + PR: canonical `AM.md`, `/design:map`, skill,
      6 хуков, V-AM-* в `design-artifact-validate.js`, merge `artifacts/README.md` (LESSON + AM = 24),
      счётчик-sweep, smoke-кейсы.
- [ ] **Шаг 3 — open-design generator DEC.** Решить судьбу CNT-004 в каноне; при принятии —
      канонизировать od-адаптеры в repo `adapters/`, dispatch в `design-session.md`, enum MK.md.
