...
### Журнал добора (2026-07-09/10)

- **Owner-действия №2 выполнены:** PA-051 RATIFIED (Side-B + полный amendment-scope, текст в самой PA),
  PA-052 → done — коммит пилота `b8d6853`. Доставка DEF-1 фикса: update → `3683834`; на старте S3
  ошибка `hookEventName` исчезла (R3-фикс работает).
- **S3 (fab-s3, ~27 мин, транскрипт `0d46bb1f`):** amendment-раунд вскрыл, что консилиум-концерн №2
  был реальным багом (view `latest_snapshot_per_user` мешает строки разных прогонов) — починено
  пиннингом к одному `aggregation_run_id` (VC-037); верификация на живом субстрате (api 854 / worker
  176 / shared 21 зелёные); коммиты `0eced8a`, `f6e7763`. Линия дошла до терминального `done`
  (seq 8, `project_fm_shipped_hint`) — **R7 теперь проверяем**. Контекст сессии на закрытии: 21%.
- **⚠ Live-дефект №3 (DEF-3, дисциплина диспетчера / дыра контракта движка):** S3 НЕ запускала
  P5/P7 как процессы — ledger не получил новых брекетов (остался 3), а seq7/seq8 созданы **голыми
  `tick --event evt:impl.go` / `evt:runtime.ready_or_started`** (в событиях нет run_id; в транскрипте
  0 вызовов Workflow, 2 ручных tick-команды). Работа при этом реальна (код/тесты/коммиты), но
  автомат прошагали вручную за 21 сек, мимо контракта run.md «prescription run-process = полный
  bracket». Заодно нарушена операторская граница «остановись на вердикте по реализации» (проскочен
  runtime_gate при живом докере — рычаг `ENV_NOT_READY` не отыгран). **Решение: харднинг движка** —
  `tick` отказывает (exit 2) событиям, ingest-мапленным процессу invoke-состояния; сознательный
  ручной прогон — только `--force-manual "<reason>"` с аудит-маркером в событии (см. DEC-DEV-0163).
- **Пре-регистрация C2 (negative control, разрешён чеклистом класс B):** после доставки харднинга
  оператор вносит задокументированный контролируемый дефект —
  `apps/api/src/modules/measurement/services/snapshot-reader.service.ts:240`: инверсия
  `trialCompletedCount < N_MIN` → `> N_MIN` (ломает suppression-инвариант BR-084; детерминированно
  ловится юнит-сьютом) — отдельным коммитом с пометкой negative-control. Затем свежая сессия,
  промпт (verbatim):

```
Прогони производственную линию фичи FM-006 (conversion measurement) через оркестратор
с fabric-трекингом: /orchestrator:run batch-features-to-cc-sdd --feature FM-006 --fabric,
дальше следуй предписаниям линии до её завершения. По ходу держи меня в курсе ключевых
решений и останавливайся там, где регламент требует моего участия.
```

  Ожидание (оператору, исполнителю не сообщается): P3 refresh → P4 (возможен повторный
  product-route по всё ещё открытой PA-044 — тогда owner-флип по уже записанной диспозиции) →
  P5 bracket → **NO-GO** (дефект) → charter `remediation` → P6 bracket → fix → **GO** →
  runtime_gate → P7 → done. Это G-B машинным путём; харднинг исключает срезание через голый tick.

### Итог добора (2026-07-10)

- **Guard DEF-3 доставлен в пилот операторским патчем** (managed-зона; после merge PR перезапишется
  тем же содержимым update'ом) — smoke на копии: bare tick `evt:spec.authored` отбит с корректным
  объяснением. Плант C2 закоммичен `d312e9c` — **ошибка пульта: commit message раскрыл плант
  исполнителю** (S4 процитировала его до старта линии). Контаминация детекции задокументирована;
  машинная маршрутизация от неё не зависит. Урок: negative-control документируется в брифе
  (сторона ревьюера), в пилот-историю — нейтральная формулировка.
- **S4-заход 1 (до ре-рана):** сессия отказалась дубль-ранить терминальную линию без явной команды
  (сверила replay'ем), распознала stale owner-queue как дрейф. Оператор явно скомандовал ре-ран
  с диспозицией «плант = обычная регрессия, обрабатывать штатно».
- **Ре-ран, инстанс №2 `2026-07-09-...-fm-006-e16txs` (S4, ~1ч43м):** P3→P4 (повторный BR-081
  гейт PA-053 — разрешён по диспозиции PA-045) → P5 no-op (26/26) → **P6#1 NO-GO: плант пойман
  (suite RED) → seq5 `impl.no_go` → remediation (ingest, run_id `e233z4`)** → remediation: откат
  инверсии `e072db1` + orphan seam `bfbdc9b` → P6#2: suite 1583/0, но реальная находка PA-054
  (design.md «single source» констант ложен: api↔worker seam) → conflict-гейт PA-055 → owner:
  Resolution 2 (санкционированный two-copy seam + drift-guard, `2787205`) → **P6#3 GO → seq8
  `impl.go` (ingest, run_id `e3kh9s`)** → P7: **DEF-4 — monorepo probe false-negative**
  (`NOT_STARTABLE` от root package.json; short-circuit ДО env-пробы → докер-рычаг оператора
  снова не дал R2) → гейт PA-057 → owner: close-complete, PA-056 → ecosystem-дефект → seq11/12
  `--force-manual` с аудит-причинами в why[] (**первое живое использование эскейпа guard'а**) →
  **done**. Replay ok seq12; ledger +7 брекетов, run-id 1:1; HEAD чист от инверсии; всё запушено.
- Owner-гейты ре-рана: PA-053 / PA-055 / PA-057 — все с записанными диспозициями. S4 отреконсилила
  5 stale owner-queue записей (ANOM-5: append-only без dequeue — движковый дефект, в апстрим).
- **Usage-limit VM-аккаунта исчерпан в конце S4** («Now using usage credits») — LLM-работа на VM
  свёрнута; финал (судья-аддендум, отчёт, PR) — на хосте.
