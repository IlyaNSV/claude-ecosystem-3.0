...
### E6 — doc-UX Волна 2 остаток *(параллелизуемо с E5; независимый трек)* — **✅ ЗАКРЫТ 2026-07-11 (DEC-DEV-0188)** (кроме C4 — остаётся за гейтом D-7)
- [x] **C5 «объясни путь»** ✅ — двух-кликовый режим (СТАРТ→ЦЕЛЬ) в карте процессов: BFS по flow-рёбрам
      (`sequence`+`delegate`) по статическим `DATA.edges` (устойчив к expand-состоянию), подсветка
      `.pathnode`/`.pathedge`, API `window.__procmap.explainPath/highlightPath/setPathMode`, кнопка «🧭 Объясни путь»/Esc.
      Smoke: путь P1A→P6o = 8 узлов, непуст, монотонен по x, направлен.
- [x] **B2 анатомия** ✅ — редакторская coarse Mermaid (D-6=b, как docs/MAP): 4 модуля × D1–D6 × handoff-граница
      × вердикт `P3→P6` в L0 хаба `docs/guide/README.md` + coarse-freshness caption (Mermaid валидирован).
- [x] 3 consistency-фикса E2-аудита ✅: ① `05-implementation.md §1` — разводка «почему `P3–P6`, а не `P1–P7`»
      (P1/P2 внутр. подготовка, P7 = нога внутри вердикта P6); ② node-id `P4`→`P4p`/`P5`→`P5p` в
      `process-graph.overlay.json` (+ alt-ref, + реген карты, + smoke-гард); ③ `NFR`=🟠/`AM`=🟢 в
      `00-concepts §5` + `06-gates §1` (сверено с glossary/overlay SSOT).
- [ ] **C4 «ты здесь»** — 🔒 **ОСТАЁТСЯ ГЕЙТОМ D-7:** строить ТОЛЬКО после 🧪 пилот-валидации Волн 0–1
      (owner-решение §5.8, дефолт «отложить»). В этом заходе НЕ строился; дизайн сохранён в
      DOCS_UX_BATCH_DESIGN §C4. C4 = read-only `state.json` против `.product/`.
- **CP-6: ✅** — `npm run verify` EXIT=0; приёмка §8 DOCS_UX_BATCH_DESIGN по Волне 2 пройдена (анатомия в
      L0; «объясни путь» даёт непустой монотонный путь P1A→P6o; аудит словаря — фиксы ①②③). C4 отложен по D-7.
