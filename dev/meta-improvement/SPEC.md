...
> **Mechanisms (v1.0):** [`CONVENTIONS.md`](CONVENTIONS.md) §1-§10 + [`checklists/phase-{kickoff,closure}.md`](checklists/) + [`patterns/`](patterns/) (**9** на 2026-07-28; **ростер — SSOT [`patterns/README.md`](patterns/README.md)**. Число здесь держится дисциплиной: `check-counts` вычисляет и печатает ground truth паттернов, но **kind `pattern` в нём заведён НЕ был — сознательно**, «pattern» в этом репо омоним (D7-паттерны / словарь анти-паттернов / regex), см. шапку `scripts/check-counts.js`. Прежнее значение «8» разошлось с диском именно поэтому) + [`skills/`](skills/) (`memory-sync` · `informed-fetch` · `repo-hygiene`) + [`scripts/verify-update.sh`](scripts/verify-update.sh) + warn-хуки [`hooks/`](hooks/) (`dev-journal-reminder` · `memory-drift-reminder` · `d7-hygiene-reminder`; `phase-closure-reminder` удалён 2026-07-28 — CONVENTIONS §13).
...
> **⚠ Frozen-snapshot caveat (DEC-DEV-0083):** §6 open-questions и §7 reference-пути ниже — историчны (часть путей вела в `dev/` root до gates-reorg). Текущая операционная истина = `CLAUDE.md` «Process triggers — harness contract» + `CONVENTIONS.md` + `DEV_JOURNAL`. «Structural growth complete» перекрыто: DEC-DEV-0083 добавил enforcement-механизмы (блокирующий `commit-msg` gate + `check-counts.js`) осознанным решением владельца. **Глубокая разморозка SPEC — вопрос ЗАКРЫТ, а не отложен** (приведено в соответствие 2026-07-28): work-order, в который она откладывалась, исполнен — `dev/_archive/meta-improvement/D7_DEADWEIGHT_CLEANUP.md` (✅ EXECUTED 2026-07-11, DEC-DEV-0185), и его собственный вердикт по этому пункту: *«frozen-секции SPEC.md и refinement-трекеры §10 — low-prio COMPACT-таргеты, сознательно оставлены (их шапки уже переадресуют истину в CLAUDE.md/CONVENTIONS)»*. Т.е. разморозка **осознанно отклонена** как не окупающаяся, а не ждёт очереди. Висячего обязательства в закрытом контейнере больше нет; если разморозка понадобится — это новая единица работы с новым решением.
...
- **DEV_JOURNAL.md** (root) — cross-session decision memory; DEC-DEV-NNNN entries (continuing sequence — current max in DEV_JOURNAL.md, past 0080). Pattern proven через multiple phases.
...
- [dev/_archive/phase-3/PHASE_3_SMOKE_TEST_PLAN.md](../_archive/phase-3/PHASE_3_SMOKE_TEST_PLAN.md) — validation pattern (archived post-closure)
- [dev/_archive/phase-4/PHASE_4_READINESS.md](../_archive/phase-4/PHASE_4_READINESS.md) — kickoff pattern (archived post-closure)
