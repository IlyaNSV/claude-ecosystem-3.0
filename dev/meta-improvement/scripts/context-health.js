 ...
 *
 * ⚠ ОТСУТСТВИЕ ДОКАЗАТЕЛЬСТВА ≠ ДОКАЗАТЕЛЬСТВО ОТСУТСТВИЯ (D4, 2026-07-13). Резолв
 * cross-repo хешей ЗАВИСИТ ОТ МАШИНЫ: на хосте владельца соседние репо лежат рядом на
 * диске, на VM / в CI / у другого разработчика — их НЕТ. Объявить хеш мёртвым можно
 * ТОЛЬКО обыскав все места, где он мог бы жить. Если хоть одно недоступно — пространство
 * поиска НЕПОЛНО, и отрицательный результат НЕ является доказательством смерти.
 * ⇒ нет ожидаемого соседнего репо на диске ⇒ класс deadPointer ОТКЛЮЧАЕТСЯ ЦЕЛИКОМ,
 * хеши идут в unverifiable. Гейт не имеет права падать на том, чего он не может знать.
 * (Эмпирика: без соседей на диске сторож объявлял «17 мёртвых» — все 17 оказались ЖИВЫМИ
 * коммитами пилота и meta-system, т.е. 17 из 17 = ложь. `npm run verify` на VM падал.)
 *
 * Использование:
...
// Соседние репо, на коммиты которых контекст ссылается ЛЕГАЛЬНО (держать в синхроне с CROSS_RX).
// Если ХОТЬ ОДНОГО из них нет на диске — пространство поиска неполно, и «хеша нет нигде» перестаёт
// быть доказуемым утверждением: хеш мог жить именно в недостающем репо. См. шапку файла.
const EXPECTED_SIBLINGS = ['meta-system', 'product-radar', 'factory-conductor', 'my-first-test'];
const absentSiblings = EXPECTED_SIBLINGS.filter((n) => !siblings.has(n));
const CAN_PROVE_DEAD = absentSiblings.length === 0;
...
const findings = {
  ...
  unprovable: 0,     // ← из них: понижены из deadPointer, т.к. соседнего репо нет на диске
};
...
function scan(file, label) {
  ...
  for (const m of txt.matchAll(/`([0-9a-f]{7})`/g)) {
    ...
    if (!CAN_PROVE_DEAD) { findings.unverifiable++; findings.unprovable++; continue; }
...
if (JSON_OUT) {
  console.log(JSON.stringify({
    residentKB: kb(residentBytes), budgetRows, memRows, findings, lies, problems,
    canProveDead: CAN_PROVE_DEAD, absentSiblings, siblingsChecked: [...siblings.keys()],
  }, null, 2));
...
console.log('│ ✗ МЁРТВЫЕ УКАЗАТЕЛИ — хеша нет НИ В ОДНОМ репо на диске: ' + dead +
            (CAN_PROVE_DEAD ? '' : '   [КЛАСС ОТКЛЮЧЁН]'));
findings.deadPointer.slice(0, 12).forEach((d) => console.log('│     ' + d.hash + '  ← ' + d.where));
if (dead > 12) console.log('│     … ещё ' + (dead - 12));
if (!CAN_PROVE_DEAD) {
  console.log('│     ⓘ соседних репо НЕТ на диске: ' + absentSiblings.join(', '));
  console.log('│       их коммиты отсюда непроверяемы ⇒ ' + findings.unprovable + ' хеш(ей) понижен(о) в unverifiable,');
  console.log('│       а НЕ объявлено мёртвыми: отсутствие доказательства ≠ доказательство отсутствия.');
  console.log('│       Полная строгость — только там, где все соседние репо лежат рядом.');
}
console.log('│');
...
console.log('│     unverifiable (другой репо / пилот / VM / зона пользовательского проекта): ' + findings.unverifiable +
            (findings.unprovable ? '   [в т.ч. ' + findings.unprovable + ' — соседний репо не на диске]' : ''));
console.log('│');
