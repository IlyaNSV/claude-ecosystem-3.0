...
Если патч менял число артефактов / правил валидации / тип-схем — пройди canonical-counts
sweep (см. memory `reference_pmo_canonical_counts`): синхронизируй счётчики во всех ~10
доках (artifacts README, validation.md шапка, и т.д.). Самый частый источник дрейфа при
cut'е.
...
- `ecosystem_version` в пилотах подтянется из первой `[X.Y.Z]` CHANGELOG при ближайшем
  `bootstrap`/`update` (штамп автоматический — `update.md`).
