...
К моменту cut'а `[Unreleased]` уже должен быть полон — что именно обязано в него попадать
(триггеры + словарь секций `Added | Changed | Fixed`) — **SSOT-таблица `CLAUDE.md`
§«Process triggers»**, принуждает `process-gate.js`; здесь правила НЕ дублируются
(копия с квантором «каждое смёрженное изменение» была строже кода и уже уводила
исполнителя — урок 70; схлопнуто, DEC-DEV-0220-e). Cut НЕ занимается восстановлением
пропущенных записей — если `[Unreleased]` неполон, сначала допиши его (это разработческая
дисциплина, не работа cut-ритуала).
...
3. Над ней создай **свежий пустой** скелет (секции по словарю SSOT-таблицы — добавляй
   `### Changed` и остальные по мере надобности):
   ```markdown
   ## [Unreleased]
