...
Если патч менял число артефактов / правил валидации / тип-схем — запусти **детерминированный
реконсилятор** (DEC-DEV-0082):

```bash
node dev/meta-improvement/scripts/check-counts.js
```

Он берёт ground-truth (артефакты — из файлов `docs/pmo/artifacts/`, правила — из SSOT
`validation.md`) и флагует все live-доки, где число разъехалось. **Зелёный = consistent.**
Этот же скрипт запускается блокирующим `commit-msg` gate'ом — красный `check-counts` блокирует
cut-коммит. Если что-то нашлось — синхронизируй (memory `reference_pmo_canonical_counts` —
список носителей). Самый частый источник дрейфа при cut'е.
...
- `ecosystem_version` в пилотах **перештампуется** из первой `[X.Y.Z]` CHANGELOG при
  ближайшем `bootstrap` (Step 7) или `update` (Step 5c re-stamp — DEC-DEV-0082; до 0082
