...
| G04 | Generic await→resume для fabric-линии работает: parked-состояния + resume-события + PA-мост (live: `awaiting_product` PA-051/052 → консилиум → resume, DEC-DEV-0162). **OD7 capability-контур ПОСТРОЕН (DEC-DEV-0171):** §6 BLOCK паркует линию (`awaiting_capability`/`awaiting_capability_impl`), capability-spec едет payload'ом в PA, resume = bracket-ре-ран (P5 продолжает с tasks.md-чекбоксов) | **Live-прогон 2026-07-10 = условный GO** (парковка `awaiting_capability_impl` + payload + resume + continuation PASS; R1 FAIL-по-букве — park-ран raw-Workflow'ом без ledger-брекета; ветка `env.up` осталась невалидированной; `OD7_LIVE_RUN_GRADE_REPORT.md`) |
