...
> ## ✅ ПРАВКИ ПРИМЕНЕНЫ — ИСПОЛНЕНО, НЕ ЗАДАЧА
>
> **Статус: EXECUTED 2026-07-17** (DEC-DEV-0220-e, коммит `a172a4b`, PR #226; поставка — релиз
> `1.12.2`). Реестр отработан целиком: кластеры terminology (4), spec-vs-impl (5), stale-refs
> (5 live + precedence-dup-5), B.1 ×4 скилла, `PHASE_4` R1 → `[FIXED]`. Трек когерентности
> закрыт — `dev/coherence/SEAM.md` `status: CLOSED`.
>
> **Что это значит для читателя.** Ниже — **исторический артефакт**, а не очередь работ.
> Императивы «применить правку», «проверь `grep -cF`» относятся к прогону 2026-07-17 и уже
> отработаны. `grep -cF` по `old_string` сейчас закономерно вернёт `0` — это **признак
> применённой правки, а не дрейфа**; не диагностируй по нему проблему и не применяй реестр
> повторно. Ценность файла — воспроизводимая улика того, ЧТО и ПОЧЕМУ было изменено
> (обоснования у каждой правки — они не дублируются больше нигде).
>
> *(Статус приведён в соответствие с фактом 2026-07-28, гигиена репо: файл 11 дней держал
> императивный тон живой задачи, хотя знание о применённости жило только в соседнем `SEAM.md`.)*

> **Что это было.** Готовые к применению правки, **пере-верифицированы против `main` @ `7ad4888`** восемью субагентами (opus, read-only) 2026-07-17. Каждый `old_string` скопирован **байт-в-байт из файла** и проверен на уникальность (`grep -cF` = 1).
>
...
>
> **Дисциплина применения (историческая).** Реестр снят на другой ветке — перед каждой правкой проверялось, жива ли она (`grep -cF '<old_string>' <файл>` → `1`). Координаты строк не использовались — они дрейфуют; матч по тексту.

---

## ⚠ Контекст прогона (было актуально до применения 2026-07-17)
