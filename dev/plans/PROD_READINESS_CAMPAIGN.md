...
| **FIND-8** | **🔴🔴 ДЕДЛОК E.A→E.B — первый деплой невозможен В ПРИНЦИПЕ** | `deploy-to-stage.mjs:214` (`manifestOk = present && status !== 'draft'`) + промпт парсера `:211` («status draft-only → `present:false`») | **E.A обязан оставить CNT `draft`** (его собственное правило: флип ждёт live-verify) · **E.B отказывается деплоить не-`active`** → `ENV_NOT_READY` → `BLOCKED` · **live-verify возможен только через успешный деплой** ⇒ замкнутый круг. Деплой закоротило **до вызова резолвера** (`disposition: null`) ⇒ **мутирующая ветка §3.2 осталась НЕПРОТЕСТИРОВАННОЙ.** **Корень — категориальная ошибка:** `draft` смешан с readiness. Оснастка НА МЕСТЕ (файл 254 стр., парсится, step-list полный) — «не смог подготовить среду» про неё неправда; правда — «контракт не подтверждён живым прогоном», а это ось **доверия/автономии**, а не готовности среды. **Почему не поймали repo-side:** обе стороны по отдельности корректны и юнит-покрыты — дефект живёт **в стыке** и виден только на первом реальном деплое | 🔧 в работе (DEC-DEV-0201): развести оси (`present` ≠ `status`) + contract-guard в резолвере (`draft` понижает `auto → human-gate`, floor неприкосновенен) + явная санкция человека на первый деплой |
| **FIND-9** | **⭐ Улов P7-гейта: 865 зелёных тестов над не-бутящимся продом** | 2 DI-дефекта в пилоте (`EMAIL_PORT` не в `exports[]`; rate-limit-гвард просил стор по class-токену, а тот провидится под строковым). Nest падает на **первой** нерезолвимой зависимости ⇒ дефекты снимаются слоями | **Это не дефект экосистемы — это её УЛОВ, и сильнейший аргумент за P7.** Почему тесты молчали: под vitest/esbuild нет `emitDecoratorMetadata` ⇒ Nest-enhancer видел **ноль** ctor-параметров ⇒ конструировал гвард с `store === undefined` **без DI-ошибки**, а fail-open `catch` в `canActivate` (NFR-007) глотал вытекающий TypeError ⇒ **тихий admit**. Под прод-сборкой (`nest build`, tsc, metadata ON) paramtype эмитится → Nest резолвит class-токен → **краш**. **Дефект существовал ТОЛЬКО в прод-сборке.** Побочно: rate-limit-троттл (NFR-007) в интеграционных тестах был **фактическим no-op** — security-контроль не проверялся вообще, хотя тесты по нему зелёные. **Урок для экосистемы: suite-green ≠ prod-boot; P6 (build+тесты) не заменяет P7 (живой boot) — и именно поэтому D-7 ставит их разными гейтами** | ✅ починено в пилоте (`e602110`, `43361c4`); api бутится, `GET /health` → **200**, полный сьют 865/865 PASS |
