...
> **Статус:** 🏁 **ЗАВЕРШЕНА 2026-07-15** (решение владельца; старт 2026-07-13, DEC-DEV-0198). Итог: Epic E deploy/rollback-контур live-валидирован (смоук S1-S7 PASS, первый живой деплой + сердце fail→auto-rollback + первое живое консилиум-жюри), релиз **1.12.0** cut и доставлен, все догоны Волны 1 PASS, graduation-порог субстрата 4/4, пост-кампанийные решения владельца №1-4 исполнены (вкл. фиксы DEC-DEV-0214/0215). Архивация файла — следующим deadweight-sweep (ссылки из ROADMAP/памяти пока живут сюда). Шов `dev/plans/SEAM.md` → CLOSED.
...
## 1-bis. Решения владельца (2026-07-15, пост-cut 1.12.0) — закрытие вопросов раунда 3

| # | Вопрос | Решение |
|---|---|---|
| 1 | Судьба ветки пилота `epic-e-live` | 🧭 Делегировано мне → **merge в main пилота + удалить ветку** (фиксы BR-079/DI и sync-коммиты обязаны жить в main; run-records закоммичены как evidence перед merge) |
| 2 | Прунинг `releases/` | 🧭 Делегировано мне → **отложен до реальной боли** (3 релиза — копейки; механизм преждевременен, cuttable scope) |
| 3 | Возврат Hyper-V | ✅ **Не возвращать** — отходили осознанно (VT-x для VirtualBox; цена WSL2/Docker принята); вопрос снят, пока VM в работе |
| 4 | Дрейф VM `cpus=4` (было 12) | ✅ **Осознанное решение владельца** — не трогать (память env обновить) |
| 5 | CNT-005 draft→active | ✅ **«Если готов — давай»** → флип исполняется Integrator'ом по live-evidence `l62yt4` |
| 6 | Ghost-подсказки TUI с action-директивами | ✅ Подсказки ≠ вымеренный механизм — им не доверять; **но возникающие в процессе решения/задачи полезно фиксировать сессионно в журнал** (не в формате подсказок) → бэклог-кандидат экосистемы: **session task capture** — сессионная фиксация появившихся по ходу решений/задач в журнал/PA (meta-feedback; не строить без отдельного решения) |
...
- **2026-07-15 (раунд 3 ЗАКРЫТ: S5 PASS с третьего захода → cut 1.12.0)** — Фикс 0211 смёржен (PR #205) и доставлен, но ре-ран S5 №2 (`ld6vrs`) сорвал **сам диспетчер**: в pre-launch «Substrate prep» поднял погашенный Redis ДО env-readiness — **FIND-F**: 0211 закрыл шесть стадий, а диспетчер `run.md` остался последним актором без capture-don't-fix (урок 56: «закрывая класс — перечисли АКТОРОВ, не места в коде»). **Фикс DEC-DEV-0213** (PR #206): блок «SUBSTRATE IS READ-ONLY FOR THE DISPATCHER» в Pre-flight + grep-инвариант. Доставка `2a5f5c5` (sync `623baf2`) → **S5 заход 3 (`le56c8`): PASS ровно по ожиданию** — 43 секунды, исполнена ТОЛЬКО env-readiness, `BLOCKED × ENV_NOT_READY / env-not-ready-preflight`, `flipped:false`, `disposition:null`, Redis поимённо, **диспетчер субстрат не тронул** (StartedAt не сдвинулся, ноль docker-вызовов; его сводка цитирует новое правило). Один прогон подтвердил живьём 0211-E1 + 0211-E2 + 0213. **Смоук Epic E закрыт целиком (S1-S7), триггер §11 Шаг C выполнен → cut 1.12.0** (CHANGELOG [1.12.0], ROADMAP-снапшот, тег; этот PR). Redis поднят обратно координатором, health 200. Остаток кампании (догоны Волны 1: E4 · E5-B · E1 · E2 · E6) — event-driven после cut. Пост-cut хвосты: graduation-правки `EPIC_E_READINESS.md` (E.D/E.G done) + `SUBSTRATE_GRADUATION_GATE.md` (компонент 2 — run-ledger live-валидирован: 5/5 прогонов раунда с полными исходами); вопросы владельцу — в шве (§Вопросы: судьба `epic-e-live`, прунинг релизов, возврат Hyper-V, дрейф VM cpus=4→12).
- **2026-07-15 (🏁 ВОЛНА 1 ДОГНАНА ЦЕЛИКОМ — кампания исчерпана по всем волнам)** — Пост-cut хвосты + все 5 догонов за один заход: **хвосты** — CNT-005 → `active` (Integrator по канону, `464f34f`, evidence l62yt4) · `epic-e-live` слита fast-forward в main пилота и удалена (решение делегировано) · прунинг релизов отложен до реальной боли (решение делегировано) · run-records раунда закоммичены как evidence. **E4 impl-sync: PASS** (сенсор 7=7 · dry-run сошёлся · гейт: N = md5 байт-в-байт, Y = spec-чистый `impl_sync` только в FM-006; advisory-находка: сенсор кредитует `fabric_done` от раннего инстанса при aborted-линии). **E5-B консилиум Tier B: PASS — ПЕРВОЕ ЖИВОЕ ЖЮРИ** (`L2×staging → consilium-gate` детерминированно → 3 плеча velocity/fidelity/integrity, opus, фикс-модель → **strong conf 1.0** → auto → DEPLOYED `20260715T112801Z`; safe-fallback честно не спровоцирован — owner-queue пуст, механизм split/none/soft-veto задокументирован; **находка: гейт читает status из deploy-manifest.yaml, а не из CNT** — после флипа CNT→active `--accept-draft-contract` всё ещё нужен, рассинхрон двух носителей → фикс-кандидат). **E6 `/integrator:verify`: PASS** (все оси на реальном дрейфе, 0 false positives: stitch D3 · open-design D1+down · deployer stale→stamped · unattributed пойман · 7 handoffs stale; **DEF-SMK-1 подтверждён закрытым живьём** — D1-D3 оси работают через CNT-резолв на реальной схеме, фикс 0178 держит). **E1 (1.3.3): S2 PASS 4/4 · S5 PASS** (dedup 1-мин конклюзивно: 4 ключа↔4 PA) · **S4 PARTIAL** — N/A снят (полный add-поток работает), но **§4.2.1-deliverable (env-блок в propose + prod-only warning) структурно не подключён в `add.md`/`installation-protocol`** — SPEC-мандат есть, wiring нет; решение владельца: чинить или признать вытесненным Epic-E provisioning; попутно: research-флоу чистит маркер у ПРЕЗЕНТАЦИИ гейта, не после резолюции (класс исходного N/A). **E2 (PHASE_6): S1 PASS · S3 PASS** (свежая честная UI FM-008 на изолированной ветке `e2-smoke-ui-fm` пилота — deviation задокументирована, outputs произведены реальным прогоном; S3 сильнее плана: генераторы реально мертвы, цепочка деградировала до html + PA-069). Статус-баннеры обоих планов и реестр CLAUDE.md обновлены; graduation: **EPIC_E_READINESS → ЗАВЕРШЁН**, **SUBSTRATE_GRADUATION_GATE → 4/4 зелёные, порог пройден** (компонент 2: 6 прогонов раунда, 0 null-исходов). Мелкие находки в очередь: gh-токен VM невалиден (нужен `gh auth login`) · malformed XML-теги в observed healthcheck-релея · невоспроизведённая PostToolUse hook error ×2 · «empty first turn» дизайн-сессии. **Кампания исчерпана по всем заявленным волнам — готова к закрытию решением владельца** (открыт только S4-gap §4.2.1 и фикс-кандидат manifest-vs-CNT).
- **2026-07-15 (🏁 ЗАКРЫТИЕ КАМПАНИИ — решения владельца №1-4 исполнены)** — Владелец согласовал все 4 рекомендации разбора: **(1) S4-gap → фикс DEC-DEV-0214** (PR #213): Stage-2 propose `/integrator:add` рендерит `environment_tiers` + обязательный prod-only warning до approve-гейта (урок 57: MUST-поле обязано пройти весь путь «собрано → показано», не только точку валидации); **(2) manifest-vs-CNT → фикс DEC-DEV-0215** (PR #213): contract-status резолвится из CNT-файла как SSOT с фолбэком на манифест + провенанс `contract_status_source` (урок 58: читать канон, копию низвести до явного фолбэка) — живьём проверено на паре пилота (CNT active × манифест draft ⇒ status=active из cnt); **(3) кампания ЗАКРЫТА**, шов → CLOSED, архивация файлов — следующим deadweight-sweep; **(4) gh-токен VM починен** неинтерактивно (`gh auth login --with-token` из env-токена пилота, scopes repo/workflow; `gh auth setup-git` — push работает штатно). Оба фикса доставлены в пилот (`/ecosystem:update` → verify Healthy). Остаточные мелочи вне кампании: opaque migrate в манифесте (до-фикс при следующем провижне) · malformed XML-теги в observed healthcheck-релея · невоспроизведённая PostToolUse hook error · «session task capture» бэклог-кандидат (§1-bis п.6) · advisory сенсора impl-evidence (fabric_done от раннего инстанса). **Кампания «до-PROD» завершена: экосистема доведена до прод-порога как фабрика продуктовой разработки — цель DEC-DEV-0198 достигнута.**
