...
| **FIND-9** | Пилот: второй DI-дефект вскрылся из-под первого | `glossary.module.ts` провайдит store через `useFactory`, гвард просит по class-токену | Nest падает на **первой** нерезолвимой зависимости ⇒ дефекты снимаются слоями. **Это не дефект экосистемы, а её улов:** P7-гейт нашёл реальную регрессию, которую P6 (build+suite) пропустил, потому что это runtime-DI | 🔧 чинится в пилоте (субстрат прогона) |
