export const meta = {
  ...
  description: 'Wave 2 keyed bake-off projected onto FREE infra: B0 naive vs B1 disciplined, blind order-swap judge on 24-query set',
  phases: [
...
// Hardcoded per-batch (args.pick was unreliable — arrived stringified). Batch-B: fill empty/thin buckets.
...
const results = await pipeline(
  ...
  (x) => parallel([
    () => agent(B0_PROMPT(x), {label:`B0:${x.id}`, phase:'Retrieve', agentType:'general-purpose', effort:'medium'}),
    () => agent(B1_PROMPT(x), {label:`B1:${x.id}`, phase:'Retrieve', agentType:'general-purpose', effort:'medium'}),
  ]).then(([b0,b1]) => ({x, b0, b1})),
  ...
  (prev) => {
    ...
    return parallel([
      () => agent(JUDGE_PROMPT(x, b0, b1), {label:`J1:${x.id}`, phase:'Judge', schema:JUDGE_SCHEMA, agentType:'general-purpose', effort:'high'}), // A=b0 B=b1
      () => agent(JUDGE_PROMPT(x, b1, b0), {label:`J2:${x.id}`, phase:'Judge', schema:JUDGE_SCHEMA, agentType:'general-purpose', effort:'high'}), // A=b1 B=b0
