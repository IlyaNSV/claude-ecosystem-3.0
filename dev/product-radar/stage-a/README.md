...
## Расширения поиска (MCP) — upgrade-путь Stage A

Stage A v0 сознательно работает на встроенных WebSearch/WebFetch Claude Code (доступны и
субагентам workflow) — ноль настройки, достаточно для проверки ценности воронки. Известные
слепые зоны встроенного поиска для этого pack:

- пагинация и фильтры отзывов (G2/Capterra глубже первых страниц);
- RU-площадки со слабой индексацией (отзовики, тендеры, реестр отечественного ПО);
- динамические листинги (Acquire/Flippa);
- полузакрытые источники (TG-каналы) — сегодня закрываются только Tier-P inbox'ом.

**Правило подключения расширений** (bring-forward триггер, не «на всякий случай»): если lane
≥2 прогонов подряд репортит деградацию/пустоту в `coverage_gaps` — под неё подключается
инструмент. Кандидаты: MCP-серверы поисковых API (Tavily / Exa / Brave Search), scraping-MCP
(Firecrawl-класс), Reddit/HN MCP, RSS-ридер; для RU-реестров и тендеров — прямые открытые
API/выгрузки. Каждый подключённый инструмент фиксируется здесь таблицей (что, под какую lane,
какой триггер сработал). В Stage B это штатная роль коннекторов C1: MCP — один из методов
коннектора наряду с `rss|api|scrape` (CONCEPT.md §5 C1).
