...
- **Адаптация:** короткий абзац (~4-6 строк): «Экосистема — не про факт использования AI, а про то, СКОЛЬКО структуры/верификации/суждения окружает вывод; она сознательно живёт на дисциплинированном конце (agentic engineering)». Grep по `vibe|agentic engineering` даёт только dev/_archive/research-cache — в user-facing доках рамки нет.
