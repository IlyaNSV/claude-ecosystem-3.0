...
> Спутник плана [`ORCHESTRATOR_DOGFOOD_PLAN.md`](ORCHESTRATOR_DOGFOOD_PLAN.md) (DEC-DEV-0058) — план задаёт *что снимаем*, этот файл — *что сняли*.
...
> **Связь:** DEC-DEV-0068 (этот harvest + правки SPEC v1.0-draft).
...
> **SSOT прогресса** доработки модуля по результатам аудита. Обновляется по мере landing'а каждого шага. Источники follow-up'ов: DEC-DEV-0068 §Follow-ups + синтез (`out-synth.md` §5). Зеркалится в harness `/tasks` на время сессии. Ветка: `feat/orchestrator-dogfood-run01`.
...
| **S4** | Решить **OD10** — scope 1-го инкремента | follow-up #3 | ✅ done → **E2+E4** (DEC-DEV-0070) | (journal) |
...
| **S5a** | **Build P3** `batch-features-to-cc-sdd` — **ГИБРИД** (DEC-DEV-0071): оркеструем `kiro-spec-batch` (он делает волны+dispatch+10-точечный cross-spec+fix), добавляем мост `handoff→brief.md/roadmap` (замена `kiro-discovery`) + preflight C-07 + детерминир. `coverage-oracle` + durable Workflow-скелет. Дропнуты как дубль: spec-author/cross-spec-reviewer/spec-fixer + arbitrate-cross-spec. `build-steering`→делегирует `kiro-steering`. Smoke на fixtures зелёный (`npm run verify` exit 0: adapters 4/4, oracle 6/6, workflow-smoke 2/2). | follow-up #4 | ✅ done | (этот коммит) |
| **S5b** | **Build нативный P5** `feature-to-tdd-impl` (OD9: Workflow читает tasks.md, диспатчит implementer'ов сам — `/kiro-impl` `disable-model-invocation`). Методологию implementer/reviewer **лифтить** из `kiro-impl`/`kiro-review` (урок DEC-DEV-0071, не писать с нуля); `gate-risk-classifier` (P0-2 design) подключается тут. После S5a-smoke. | follow-up #4 | ⬜ pending | — |
| **S6** | Прогон №2 с **неоснащённой** инфрой — провалидировать §6 на D3 (откр. вопрос №1; важность ↑ т.к. E4 в scope) | follow-up #5 | ⬜ pending | — |
