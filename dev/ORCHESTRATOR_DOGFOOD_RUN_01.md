...
| **S5a** | **Build P3** `batch-features-to-cc-sdd` (роли inline per D.1; build в репо; smoke на fixtures). Порядок генерации — kickoff §E. | follow-up #4 | 🔄 in-progress (next action) | — |
| **S5b** | **Build нативный P5** `feature-to-tdd-impl` (gate-risk-classifier, снять `disable-model-invocation`/OD9). После S5a-smoke. | follow-up #4 | ⬜ pending | — |
