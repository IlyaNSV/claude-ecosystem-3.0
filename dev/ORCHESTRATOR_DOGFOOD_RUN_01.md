...
| **S7** | Прогон №3 — **detect-leg** §6 (#3 env-probe provider-секреты + `EXPECTED-ABSENT-BUT-DEFERRED`; #4 tracking-rule; + OD7 await) на неоснащённом ПРОВАЙДЕР-пробеле; «реализовать-затем-валидировать» | DEC-DEV-0081 остаток | ⬜ pending (бриф `dev/ORCHESTRATOR_S7_BRIEF.md`) | — |
