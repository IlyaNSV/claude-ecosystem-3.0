...
> Спутник плана [`ORCHESTRATOR_DOGFOOD_PLAN.md`](_archive/orchestrator/ORCHESTRATOR_DOGFOOD_PLAN.md) (DEC-DEV-0058) — план задаёт *что снимаем*, этот файл — *что сняли*.
...
| **S3** | `gate-risk-classifier` (**P0-2**) + реестр load-bearing инвариантов — **design** (`dev/_archive/orchestrator/ORCHESTRATOR_GATE_RISK_CLASSIFIER.md`, валидирован 16/17 против RUN 01; wiring отложен в S5 per OD10) | follow-up #2 | ✅ design done | `5350a6d` |
| **S4** | Решить **OD10** — scope 1-го инкремента | follow-up #3 | ✅ done → **E2+E4** (DEC-DEV-0075) | (journal) |
| **S5-kickoff** | D7 phase-kickoff build P3+P5: решения A1–A6, scope-cuts, split, Q1–Q4 подтверждены, harness-ограничение D.1 | follow-up #4 | ✅ done (`dev/_archive/orchestrator/ORCHESTRATOR_BUILD_KICKOFF.md`) | `a6078fc` (+resolutions) |
...
| **S7** | Прогон №3 — **detect-leg** §6 (#3 env-probe provider-секреты + `EXPECTED-ABSENT-BUT-DEFERRED`; #4 tracking-rule; + OD7 await) на неоснащённом ПРОВАЙДЕР-пробеле; «реализовать-затем-валидировать» | DEC-DEV-0081 остаток | ✅ done — **STRONG PASS** (§6 detect-leg флипнул BLOCK→SATISFIED + OD7 end-to-end; закрыл #3/#4) | DEC-DEV-0126 / PR #89 |
| **P2-dogfood** | Live-прогон P2 `decide-architecture-foundation` на ДВУХ реальных cross-spec-conflict форках (`PA-040` транспорт стадий / `PA-042` владение wiring'ом провайдера) — валидирует консилиум-синтез (гетерогенность жюри / детерминир. matrix+rank+veto / выпяченный split / граница «готовит-не-решает» FB-LR-07) на живых данных; **закрывает open-Q#7** | DEC-DEV-0129 build; open-Q#7 | ⬜ pending (бриф `dev/_archive/orchestrator/ORCHESTRATOR_P2_DOGFOOD_BRIEF.md`) | — |
