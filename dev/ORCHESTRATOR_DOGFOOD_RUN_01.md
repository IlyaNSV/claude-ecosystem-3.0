...
## 9. Следующие шаги (из синтеза)

1. ✅ (этот файл) перенос harvest + DEC-DEV-0068.
2. Канонизировать **P0-1** (content-level oracle) + пропагировать фикс адаптера upstream (§7).
3. Спроектировать `gate-risk-classifier` (P0-2) + стартовый реестр load-bearing инвариантов из auth-спека.
4. SPEC-правки §6/OD5 на реальных кейсах (типология +env-constraint/secret, `route`-поле, раздельные tier'ы, async-протокол).
5. Прототип **P5 как нативного Workflow-скелета** (снять конфликт `disable-model-invocation`).
