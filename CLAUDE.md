...
См. [ROADMAP.md](ROADMAP.md) секцию «Где мы сейчас» — single source of truth. Snapshot на момент последнего update этого файла (2026-05-26):
...
- ✅ **Phase 5 runtime smoke + closure (DEC-DEV-0044, 2026-05-26)** — 4 PASS clean (S1/S2/S4 + S3 post-fix), S6 PARTIAL→FIXED, S5 deferred. **3 bugs fixed end-to-end:** bug 1 (skill+agent narrow heuristic for reference adapter — Glob+slug-tolerant fix), bug 2 (bootstrap/update не deploy adapters/ — tri-location pattern adopted), bug 3 (journal-hook Windows path regex separator mismatch). **Bug 4 + C-03 cosmetic deferred к Phase 5.1 patch.** Plan archived [`dev/_archive/phase-5/PHASE_5_SMOKE_TEST_PLAN.md`](dev/_archive/phase-5/PHASE_5_SMOKE_TEST_PLAN.md); forensics в `dev/_archive/phase-5/smoke-evidence/`
- 📐 **Phase D — Wiki initiative (design frozen 2026-05-26)** — интерактивная документация по самой экосистеме в стиле Confluence. MkDocs Material + защищённый Charter (immutable/append-only секции, PreToolUse hook + git pre-commit) + 3 команды (`/ecosystem:docs-init|update|verify`) + 3 skills + GH Action draft-PR auto-sync. Design: [`dev/wiki-design.md`](dev/wiki-design.md) (§1-17); Readiness gate: [`dev/PHASE_D_DOCS_WIKI_READINESS.md`](dev/PHASE_D_DOCS_WIKI_READINESS.md) (Sections A-H). **UNBLOCKED — Phase 5 closure done.** Formal DEC-DEV-NNNN open при start
- ⏳ **Next:** Phase D kickoff (per `dev/meta-improvement/checklists/phase-kickoff.md` fresh-session) → DW.A-DW.J → решение Phase 6 (conditional) vs Phase 7. **Параллельно (или в Phase 5.1 patch):** bug 4 fix + S5 re-run + C-03 generator semver-range
...
- `dev/PHASE_D_DOCS_WIKI_READINESS.md` — wiki initiative (после Phase 5 closure)
- `dev/PHASE_6_READINESS.md` skeleton — Design Module conditional (после Phase D + first UI feature)
