...
| V-23 | 🔴/🟡 | FM, MK | IF `has_ui: true` AND `status: shipped`: `impl_sync.synced_at >= 2026-07-31` → `impl_sync.visual_review` c non-empty `reviewed_on` + `verdict ∈ {conforms, deviations-accepted}` + `evidence_ref` (+ non-empty `deviations[]` если `verdict: deviations-accepted`; + non-empty `manual_reason` если `evidence_source: owner-manual`), иначе 🔴. Shipped до знака ИЛИ `impl_sync` отсутствует целиком → 🟡 grandfather. `git log -1 --format=%cs -- <MK из mockups[]>` новее `reviewed_on` → 🟡 «re-review рекомендуется»; git недоступен → **skip с note** на дата-ноге | Visual review has_ui-реализации против MK (терминатор `shipped`; DEC-DEV-0237) |

**V-20/V-21/V-22/V-23 — on-demand и approve gate, НЕ inline.** Inline-хук `artifact-validate.js` их не исполняет (см. `validation.md §11`); в общем прогоне раннера они работают полноценно. V-21 при недоступном git — **skip с note**, никогда «✅ pass»: та же семантика, что у V-MK-*/V-AM-* (анти-паттерн 4); то же — дата-нога V-23 (сравнение с git-датой MK). Approve gate для V-23 — Step 4a команды `/product:impl-sync`: без owner-вердикта `C`/`D` флип в `shipped` не выполняется.
...
- Catalog spec: `.claude/docs/pmo/validation.md` (49 rules + 3 process rules)
