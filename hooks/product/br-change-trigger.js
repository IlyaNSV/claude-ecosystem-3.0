#!/usr/bin/env node
/**
 * br-change-trigger.js — PostToolUse hook for P-RULE-02 enforcement.
 *
 * Adaptive-depth DA model per DEC-DEV-0012 + DEC-DEV-0013 #8.
 *
 * Trigger: PostToolUse Write/Edit на .product/business-rules/*.md.
 * Action:
 *   1. Detect BR file change
 *   2. Compute git diff against HEAD (best-effort — graceful if no git/no HEAD)
...
// ---------- Filter: only .product/business-rules/*.md ----------

const normalized = filePath.replace(/\\/g, '/');
if (!/\.product\/business-rules\/[^\/]+\.md$/.test(normalized)) process.exit(0);
...
const brId = fm.id;
const brTitle = fm.title || '<unknown>';
const brStatus = fm.status || '<unknown>';

// ---------- Compute git diff against HEAD (best effort) ----------
...
try {
  ...
  // No git, no HEAD, or file new — diff будет empty
  diff = '';
}

// If diff empty (file new или not в git) — flag as creation
if (!diff) {
  diff = `# (No git diff available — likely BR creation or file not in git)\n# Full file content:\n\n${content.slice(0, 4000)}`;
...
function parseDaEntriesYaml(text) {
  ...
  blocks.forEach((block) => {
    ...
    block.split(/\r?\n/).forEach((line) => {
      if (inDiff) {
        // Diff is multiline literal block (|)
